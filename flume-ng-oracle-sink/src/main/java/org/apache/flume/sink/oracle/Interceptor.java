...
import org.apache.flume.sink.oracle.utile.DataRule;
import org.apache.flume.sink.oracle.utile.EventInterceptor;
...
public class Interceptor implements org.apache.flume.interceptor.Interceptor {
    ...
    public void initialize() {
        breaks = cc.get(OracleInterceptorConfigurationConstants.SEPARATOR);
//        if (StringUtils.isEmpty(breaks)) {
//            breaks = ",";
//        }
        deleteFild = cc.get(OracleInterceptorConfigurationConstants.DELETE_FILD);//不想要的字段
        ruleType = cc.get(OracleInterceptorConfigurationConstants.RULE_TYPE);//规则类型
        fileVal = cc.get(OracleInterceptorConfigurationConstants.FILE_VAL);//数据条件//-1,0,1,2
        fileVals = StringUtils.splitPreserveAllTokens(fileVal, ",");
        fieldsContact = cc.get(OracleInterceptorConfigurationConstants.FIELDS_CONTACT);//关联原字段索引
        fieldsDateType = cc.get(OracleInterceptorConfigurationConstants.FIELDS_DATE_TYPE);//定义时间字段
        fildnames = cc.get(OracleInterceptorConfigurationConstants.FILD_NAME);
        fiedstype = cc.get(OracleInterceptorConfigurationConstants.FIELDS_TYPE);
        da = new DataRule();
