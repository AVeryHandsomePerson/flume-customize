...
public class MysqlSink extends AbstractSink implements Configurable {
    ...
    public void configure(Context context) {
        ...
        tableName = context.getString(MysqlSinkConfigurationConstants.INDEX_NAME);//落地表名
        leng = context.getString(MysqlSinkConfigurationConstants.FILDS_LENGTH);//字符长度
        type = context.getString(MysqlSinkConfigurationConstants.TYPES);//数据类型
        fildname = context.getString(MysqlSinkConfigurationConstants.FILD_NAME);//字段名称
        batchSize = context.getInteger(MysqlSinkConfigurationConstants.BATCH_SIZE, size);
        if (sinkCounter == null) {
    ...
    public synchronized void start() {
        ...
        try {
            ...
            cn = MysqlConnect.getConnection(source, url, userName, password);//获取连接
